...
class MLF_backbone(nn.Module):
    ...
    def forward(self, z_all_or, channel_mask=None):                                                                   # z: [bs x nvars x seq_len]
        norm_all={}
        ...
        z,scale_all_rec,scale_all_patch = self.backbone(norm_all,x_all_or=z_all_or,channel_mask=channel_mask)
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    ...
    def forward(self, scale_all,x_patch_or=None,x_all_or=None,channel_mask=None) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        scale_all_rec={}
        scale_all_patch={} #used to calculate reconstruction loss

        # 可以在这里加一行打印，验证一下你的 mask 是不是真的传进来了（调试用）
        if channel_mask is not None:
            pass # 这里就像留了一个接口，将来我们要在这里做文章
        ...
        # =========== [新增] 核心手术：通道融合 ===========
        if channel_mask is not None:
            # 1. 还原维度：[Batch * Channel, Patch, Dim] -> [Batch, Channel, Patch, Dim]
            # 我们假设 x 的前两个维度分别是 Batch 和 Channel
            # (注意：一定要确保这里的维度顺序和你项目里的一致，刚才我们分析过 x 是 B, C, L 变来的)
            # 在 MLF 里，进入 TSTiEncoder 之前，它把 Batch 和 Channel 合并了。

            # 这里的 B 和 N 需要从 configs 或者输入的 x 原始形状里推断
            # 但最安全的方法是利用 channel_mask 的大小，因为它一定是 [Channel, Channel]
            N = channel_mask.shape[0]
            # 总行数除以 Channel 数，就是 Batch Size
            B = u.shape[0] // N

            P, D = u.shape[1], u.shape[2]

            # View 变形
            u_reshaped = u.view(B, N, P, D)

            # 2. 应用 Mask (Matrix Multiplication / Einsum)
            # 让通道 m 吸收通道 n 的信息，根据 mask(m, n) 的权重
            if not hasattr(self, 'DEBUG_BACKBONE_MASK_ONCE'):
                self.DEBUG_BACKBONE_MASK_ONCE = True
                with torch.no_grad():
                    nz = (channel_mask.abs() > 1e-12).float().mean().item()
                    row_nz = (channel_mask.abs() > 1e-12).sum(dim=1).unique().detach().cpu().tolist()
                    # print(f'[mask@backbone] min={channel_mask.min().item():.4f} max={channel_mask.max().item():.4f} nonzero_ratio={nz:.3f} row_nz_unique={row_nz}')
            u_fused = torch.einsum('bnpd, mn -> bmpd', u_reshaped, channel_mask)

            # 3. 残差连接 (Fusion) + 重新打散
            alpha_max = float(os.getenv("MLF_ALPHA", "0.02"))
            warm = int(os.getenv("MLF_ALPHA_WARMUP_STEPS", "0"))
            if warm > 0:
                if not hasattr(self, "_alpha_step"): self._alpha_step = 0
                self._alpha_step += 1
                t = min(1.0, self._alpha_step / float(warm))
                alpha = alpha_max * t
            else:
                alpha = alpha_max
            if not hasattr(self,"DBG_ALPHA_ONCE"): self.DBG_ALPHA_ONCE=True; # if not hasattr(self,"DBG_ALPHA_ONCE"): self.DBG_ALPHA_ONCE=True; print(f"[alpha] {alpha}")
            u = ((1 - alpha) * u_reshaped + alpha * u_fused).view(B * N, P, D)
        # ===============================================
        if x_patch_or!=None:
