...
import time
import torch
SEED = int(os.getenv("MLF_SEED", "1986"))
print(f"[SEED] {SEED}")
...
def main():
    ...
    parser.add_argument('--use_duet', type=int, default=1, help='1: enable DUET, 0: pure MLF baseline')
    ...
    args.speed_mode = (os.getenv("MLF_SPEED_MODE", "0") == "1")
    ...
    args.state = 'train'
    # ---- append duet tag into data_real to avoid ckpt overwrite ----
    if hasattr(args, 'data_real') and getattr(args, 'data_real', None) is not None:
        args.data_real = str(args.data_real) + f"_duet{args.use_duet}"
    ...
    if hasattr(args, 'is_training') and int(getattr(args, 'is_training', 1)) == 0:
        if hasattr(args, 'state'):
            args.state = 'test'

    print('[PATCH] effective is_training=%s checkpoints=%s' % (getattr(args,'is_training',None), getattr(args,'checkpoints',None)))
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            exp.train(setting)
            # ---- force save last model (for reproducibility) ----
            save_dir = os.path.join(args.checkpoints, setting)
            os.makedirs(save_dir, exist_ok=True)
            m = getattr(exp, 'model', None)
            m = getattr(m, 'module', m)
            torch.save(m.state_dict(), os.path.join(save_dir, 'last.pth'))
            torch.save(m.state_dict(), os.path.join(save_dir, args.script_id + 'checkpoint.pth'))
            print('saved ckpt ->', os.path.join(save_dir, args.script_id + 'checkpoint.pth'))
            print('saved last model ->', os.path.join(save_dir, 'last.pth'))
        ...
        # [PATCH] FORCE_TEST_ELSE
        exp = Exp(args)
        for ii in range(args.itr):
            setting = f'{args.data_real}_{args.model}_{"MultiPeriod_128_384_512_768_1024"}_pl{args.pred_len}'
            print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
            exp.test(setting)
